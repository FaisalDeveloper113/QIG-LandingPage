    ...
    <>
      ...
      <Container>
        <h1
          ...
          className="z-index relative mt-100 text-center text-6xl font-bold leading-snug tracking-tigh lg:text-8xl lg:leading-tight xl:text-8xl xl:leading-tight text-white"
        >
          Blogs
        </h1>
      </Container>
          ...
          <div className="container mx-auto md:max-w-4xl max-w-xs">
            <Slider {...settings}>
              {blogs.map((blog, index) => (
                <div
                  key={index}
                  className=" border-2 border-solid border-yellow-400  bg-gradient-to-r from-gray-900 via-sky-950 to-sky-950 dark:bg-trueGray-800 shadow-4xl  rounded-lg p-6 max-w-6xl bg-opacity-0"
                >
                  <h2 className="text-xl font-semibold text-white mb-4">
                    {blog.title}
                  </h2>
                  <p className="text-gray-400 text-justify">{blog.content}</p>
                  <a href="#" className="mt-4 block text-yellow-400">
                    Read More
                  </a>
                </div>
              ))}
